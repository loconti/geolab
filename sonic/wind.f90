...
PROGRAM wind_sonic
  ...
  REAL(kind=8) :: theta=0, psi=0, phi=0
  REAL(kind=8), PARAMETER :: deg2rad=3.1415926/180.0  ! rad = gradi * deg2rad
  ...
  ! open input file
  ! access='stream' for compatibility with COUNT_LINES
  ...
  CALL OPEN_OUT (fileOut_name, fileOut, iostat=fileOut_status)
  ...
  READ (fileIn,*) ! skip blank line
  WRITE (*,*) "parameters number is: ", n_sonic_par
  ! check if number of parameters is acceptable
  ...
  END IF
   ...
   ! count number of lines in input file until blank line or EOF
   ! without shift in file position 
  CALL COUNT_LINES(fileIn, nlines, iostat=fileIn_status, error_i=error_i)
  IF (error_i /= 0) THEN
     ! generic error in COUNT_LINES
  ...
  ! read nlines from fileIn, and rotate
  DO line_i=1,nlines
     ! read one line and store in measures(line_i)
  ...
  ! write the header
  WRITE (fileOut, '("Wind measures in the laborator coordinates at ", A)') TRIM(datetime_line)
