...
CONTAINS
  ...
  SUBROUTINE COUNT_LINES(unit, nlines, iostat, error_i)
    ...
    ! error_i = 1: file not open
    !           2: file access not set to 'stream'
    !           3: returning with iostat /= 0
    INTEGER, INTENT(in) :: unit
    ...
    IF (.NOT. filecheck) THEN
       WRITE (*,*) "Worning in COUNT_LINES: file is not open"
       error_i = 1
       RETURN
    END IF
    IF (file_access /= 'STREAM') THEN
       WRITE (*,*) "Worning in COUNT_LINES: file is not set with ACCESS='STREAM'"
       error_i = 2 
       RETURN
    END IF
    ...
    ! check iostat
    IF (iostat /= 0) THEN
      WRITE (*,*) "Worning: file closed after execution of COUNT_LINES"
      error_i = 3
    END IF   
